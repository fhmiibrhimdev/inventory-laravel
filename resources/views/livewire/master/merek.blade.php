<div>
    <div class="section-header tw-rounded-lg tw-text-black tw-shadow-md tw-shadow-gray-200">
        <h4 class="tw-text-lg">Master Data - Merek</h4>
    </div>
                    ...
                    <div class="card-body">
                        <form>
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control tw-rounded-lg" name="kode_merek" id="kode_merek" wire:model='kode_merek'>
                                @error('kode_merek') <span class="text-danger">{{ $message }}</span> @enderror
                            </div>
                            <div class="form-group">
                                <label for="nama_merek">Nama Merek</label>
                                <input type="text" class="form-control tw-rounded-lg" name="nama_merek" id="nama_merek" wire:model='nama_merek'>
                                @error('nama_merek') <span class="text-danger">{{ $message }}</span> @enderror
                            </div>
                            <button type="submit" wire:click.prevent="store()" wire:loading.attr="disabled" class="btn btn-outline-success form-control tw-py-1">Save Data</button>
                        </form>
                    </div>
                </div>
            </div>
                            ...
                            <table class="tw-table-fixed tw-w-full tw-text-black tw-text-md mt-3 tw-border-collapse tw-border">
                                <thead>
                                    <tr class="tw-border-b tw-text-xs text-center text-uppercase">
                                        <th class="p-3" width="20%">Kode Merek</th>
                                ...
                                <tbody>
                                    @forelse ($data as $row)
                                        <tr class="tw-bg-white tw-border tw-text-uppercase tw-border-gray-200 hover:tw-bg-gray-50">
                                            <td class="p-3">{{ $row->kode_merek }}</td>
